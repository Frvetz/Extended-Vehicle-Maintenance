			...
			if MathUtil.vector3Length(px-vx, py-vy, pz-vz) < 4 then
				...
				if (EVMnodeToObject.PalletOriginalTime and EVMnodeToObject.PalletHoursToAdd and EVMnodeToObject.PalletDaysToAdd) ~= nil then
					...
					if sx <= 1 and sx >= 0 and sy <= 1 and sy >= 0 and sz <= 1 and sz >= 0 then
						ExtendedVehicleMaintenancePalletText.renderPalletText(sx - 0.865, sy, (g_i18n:getText("information_palletLength", ExtendedVehicleMaintenancePalletText.l10nEnv):format(EVMnodeToObject.PalletOriginalTime)) .. "\n" .. "\n" .. "\n" .. (g_i18n:getText("information_palletOperatingHours", ExtendedVehicleMaintenancePalletText.l10nEnv):format(EVMnodeToObject.PalletHoursToAdd)) .. "\n" .. "\n" .. "\n" .. (g_i18n:getText("information_palletDays", ExtendedVehicleMaintenancePalletText.l10nEnv):format(EVMnodeToObject.PalletDaysToAdd)), getCorrectTextSize(0.01235), 0.13, farmColor);
						ExtendedVehicleMaintenancePalletText.renderIcons(self.IconProposal,      {1, 1, 1, 0.87}, sx + 0.02 - 0.865, sy + 0.2);
						ExtendedVehicleMaintenancePalletText.renderIcons(self.IconTime, 	 {1, 1, 1, 0.87}, sx - 0.02 - 0.865, sy + 0.124);
						ExtendedVehicleMaintenancePalletText.renderIcons(self.IconPlus,      {1, 1, 1, 0.87}, sx - 0.02 - 0.865, sy + 0.083);
						ExtendedVehicleMaintenancePalletText.renderIcons(self.IconPlus,      {1, 1, 1, 0.87}, sx - 0.02 - 0.865, sy + 0.042);
					end
				end
			end
		end
